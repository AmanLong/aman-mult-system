...
export default {
	...
	mutations: {
		...
		// 回显 tab
		reShowHash(state, self) {
			let url = location.href
			let indexOfSharp = url.indexOf('#')
			if (indexOfSharp > 0) {
				let hash = url.substr(indexOfSharp + 1)
				let tab = state.openedTabs.find(i => "/" + i.menuId === hash)
				...
				// }
				let tabExsit = state.openedTabs.find(i => i.menuId == tab.menuId)
				if (!tabExsit) {
		...
		OpenedTabsRemove(state, menuId) {
			...
			if (!tab) {
				let nextTab =
					state.openedTabs[state.openedTabs.length - 1];
				if (nextTab) {
					state.currentTabIndex = nextTab.menuId;
				}
			}
		...
		CloseAllTabs(state) {
			...
			state.openedTabs = state.openedTabs.filter(item => {
				return item.menuId === state.homeTabMenuId
			})
			store.commit('SetHash')
		},
	},
